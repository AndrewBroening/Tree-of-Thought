...
// ===== GLOBAL STATE =====
let globalState = {
  mode: 'setup-goal',         // 'setup-goal', 'setup', 'adding', 'exploring', 'marking', 'exhausted'
  problem: {
    goal: '',
    name: '',
...
function goToSetup() {
  globalState.mode = 'setup-goal';
  globalState.problem = { goal: '', name: '', startThought: '' };
...
/**
 * Move from goal setup to starting thought setup
 */
function goToSetupThought() {
 ...
 */
function createProblem(goal, startThought) {
  if (isProcessing) return false;
  isProcessing = true;
  
  try {
    if (!goal || !startThought) return false;
    
    globalState.problem = { goal, name: startThought, startThought };
...
function addThought(thoughtText, score) {
  ...
  if (!thoughtText || thoughtText.trim().length === 0) return false;
...
function exploreThought() {
  ...
  try {
    ...
    // Move to child node in ADDING mode
...
function goToParent() {
  ...
  // Non-root layer exhausted - switch to adding mode to add more candidates
  console.log('✗ No unexplored at parent - switching to ADDING mode');
  globalState.mode = 'adding';
  notifyObservers();  // Notify to add more candidates at this layer
  return true;
}
...
function markDeadEnd() {
  ...
  try {
    ...
    if (parentNode.unexploredThoughts && parentNode.unexploredThoughts.length > 0) {
      // Parent has unexplored thoughts to explore
      globalState.mode = 'exploring';
      notifyObservers();
      return;
    }
    
    // Parent has no unexplored thoughts - switch to adding mode to add more
    globalState.mode = 'adding';
    notifyObservers();
...
window.goToSetupThought = goToSetupThought;
