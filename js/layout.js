...
function calculateZoomToFit(boundingBox, containerWidth, containerHeight) {
  const paddingBuffer = 20; // Extra padding around tree
  
  // Calculate available space
  const availableWidth = containerWidth - (paddingBuffer * 2);
  const availableHeight = containerHeight - (paddingBuffer * 2);
  ...
  const translateX = paddingBuffer + (availableWidth - scaledWidth) / 2 - boundingBox.minX * scale;
  const translateY = paddingBuffer + (availableHeight - scaledHeight) / 2 - boundingBox.minY * scale;
